...
def perform_actions(objects):
    ...
    actions = []
    msg = ''
    numCoinsDetected = len(objects.get(COIN, []))
    numPersionDetected = len(objects.get(PERSON, []))
    ...
    if (numCoinsDetected > 0 or numPersionDetected > 0) and random.random() > randomPro:
        ...
        if len(objects.get(PERSON, [])) > 0 and random.random() < runAwayProb:
            # run away from people, they can be murders!
            xm = np.mean([xy[0] for xy in objects.get(PERSON, [])])
            ym = np.mean([xy[1] for xy in objects.get(PERSON, [])])
        ...
        elif len(objects.get(COIN, [])) > 0:
            # collect coins
            x, y = random.choice(objects.get(COIN, []))
            rbx_control.release_all_keys()
